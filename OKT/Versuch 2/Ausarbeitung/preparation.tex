...
\begin{equation}
S = \frac{\eta e \lambda}{hc}
\label{eq:}
\end{equation} 
where $h$ is Planck's constant, $e$ the elementary charge, $c$ the speed of light,  $\eta$ the quantum efficiency and $\lambda$ the wavelength of the incoming light. 
Since $\alpha$ depends on the wavelength, $\eta$ depends on the wavelength as well. For $\lambda > \lambda (W_{\mathrm{gap}})$ nothing gets absorbed by the material and $\eta = 0$. So the sensitivity below is 0 for $\lambda > \lambda (W_{\mathrm{gap}})$.
 
For $\lambda < \lambda (W_{\mathrm{gap}})$ the sensitivity decreases. 
When keeping the optical power constant and decreasing the photon wavelength there are less incoming photons. Every photon can only generate on electron-hole pair. So the sensitivity decreases linear with a decreasing wavelength.\footnote[2]{Source: Optical Communication Systems - Part 2, Wolfgang Freude}
The energy of the photons is higher than the band gap energy. This additional energy of the photons transfers into phonon energy.

Figure \ref{fig:sens} shows the spectral sensitivity of a BPW34 PIN photodiode\footnote[4]{Source: Vishay Semiconductors, BPW34 Datasheet}. 

\begin{figure}%
\centering
\includegraphics[width=.35\columnwidth]{Grafiken/sensitivity.png}%
...
\begin{figure}[h]%
\centering
\includegraphics[width=.45\columnwidth]{Grafiken/OPAMP_m.pdf}
\caption{Photodetector with transimpedance amplifier.
}%
\label{fig:OPAMP}%
\end{figure}

In Figure \ref{fig:OPAMP}\footnote[3]{Source: OKT Experiment 2: Photodiode analysis task-sheet}. with Kirchhoff's second law it can be derived (using the red loop):
\begin{equation}
...
\begin{equation}
\left|\zeta\right| = \left|S\cdot \frac{R}{1+ j\omega RC}\right|.
\label{eq:}
\end{equation}
% \comseb{Hier steht noch rot "Beträge!" am rand. die beiden letzten formeln können aber ja eigentl schon complex sein. Bei der 2. kann ma sichs höchstens nochma überlegen.}
