...
For the Experiment an OptiSim Example of a 50~\% RZ-OOK-Transmitter was used. Figure \ref{fig:P1_aufbau} shows the schematic of this example \footnote[3]{Materials for the Preparation of Experiment 7: Simulation of Optical Transmitters}. In the upper left-hand side there's the PRBS which generates a pseudo-random bit-stream. A logical connection path (green) leads to an NRZ-Rectifier (recnrz1) which generates an electrical output signal. Via an MZI this electrical NRZ-signal gets modulated on a 1550~nm laser. 
...
For driving the MZ in "`push-pull"' mode (cf. \ref{sec:MZM}) the phase of one electrical paths is shifted. The folloing MZM (PulseCarver) is used to carve the NRZ pulses to a NRZ-OOK signal.
...
Figure \ref{fig:P1ExB_Eye} shows an eye diagramm of the generated NRZ signal. Since a digital signal is regarded, the slopes of the signal are perfectly steep. The measured log Q factor of the NRZ signal is 40 dB.

Figure \ref{fig:P1ExB_bit} shows the electrical , the optical NRZ  and the optical 50~\% RZ bit sequenze. Both optical sequences match the electrical signal. The RZ signal shows erroneous pulsshape, because the exported data of this signal has not enough datapoints. Figure \ref{fig:P1ExB_FWHM} shows the correct shape of one pulse. In this figure also the full width at half maximum can be measured. With an FWHM of 0.05~ns and a lenght of a bitslot of 0.1~ns the duty cycle of the optical signal is 50~\%.
...
For the Q-factor a value of 40~dB is obtained. This value is not very practical, since the Q-factor can be decreased by the photodiode as well. 
...
Figure \ref{fig:P2ExB_3dB} shows the output signal of the PIN-diode for different -3~dB bandwidths. To demonstrate this effect an incoming optical NRZ signal was used. A bandwidth of 10~GHz (cf. fig. \ref{fig:P2ExB_10}) leads to a rise-time of approximately 0.03~ns.
...
\section{Project 3}
\label{sec:P3}
In Project 3 an additional fibre was added to the simulation. The fibre "`CorningSMF28\_1550"' was inserted between the Pulse Carver and the in project 2 added PIN diode. For this project all nonlinear effects were deactivated ("`Advanced control"' -> all options were switched off).
...
Table \ref{tab:P3ExB_loss} shows the calculated and the measured losses of the fibre with different fibre lengths. The attenuation is calculated by $\alpha = \alpha_0 \cdot L$ with $\alpha_0=0.19$~dB/km and the fiber length $L$. The measured losses are a little bit smaller then the calculated values. As a more realistic approach the calculation of the attenuation is more complex, since it depends on the wavelength. Thus for a certain spectral width the attenuation is not constant. This shoud be taken into account by the simulation program.
...
To measure the impulse's temporal half-maximum full-width difference $\Delta t_{\mathrm{H}}$ a prepared bit sequence was loaded ("`costum\_1.dat"') and a parameter scan was performet for the lenght of 10, 20, 30 and 40~km.
...
\begin{table}%
\centering
\caption{Losses and temporal FWHM}
\begin{tabular}{ccccc}
...
Table \ref{tab:P3ExB_loss} shows the calculated and the measured broadening of the pulses. With a larger fibre-length the temporal FWHM difference gets larger. If compared to the calculated values, this can also be seen. But the calculated values differ from the measured ones.  The pulse shape for the calculation is approximated as a gaussian, but the real pulse is not perfectly shaped. For long fiber length the pulse broadens and thus fits better to the gaussian shape. 