...
A change of the between 1~V and 2~V showed that the highest extinction ration could be achieved at an clock voltage of 1.5~V. Changing the optical amplitude of the electrical data pulses showed that for a amplitude of 0.5~V the extinction ratio becomes small ($\sim 5.31$~dB). A higher amplitude of 1.5~V leads only to a small improvement. For that reason the amplitude of the data pulses was set to 1.0~V like in the task described.

Now at the pulse pattern generator was set into the WORD mode to send a defined bit pattern. The generator was set to send the bit sequence ``00100100''. The PAT SYNC output of the PPG was connected to the front panel input of the oscilloscope.
...
\label{eq:}
\end{equation}
Since the histograms of both mark and space were symmetric and looked Gaussian and therefore can be assumed to be Gaussian distributed and because the standard deviation of mark and space are nearly the same this equation can be used.

The calculated values can be found in table \ref{tab:B_Q}.
