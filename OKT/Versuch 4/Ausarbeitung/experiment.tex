...
\caption{Backscattered singnal with an open connector (TRACE015), with an connector not closed propperly (TRACE021) and a closed connector as reference (TRACE006)}%
\label{fig:connector}%
\end{figure}
...
Using the 4 point measurement of the OTDR the Losses for the closed connector are measured as 0.8~dB. For a not propperly closed connection a loss of 1.5~dB is measured. For a connector loosend further the losses at the connection are greater than 12~dB, so that the backscattering at the end of the fiber couldn't be detected.
...
At 21st km where APC connector was replaced with a PC a 17.7~dB high reflection peek can be seen. It is due to the fact that the PC connector has a 0° angle. This simplifies the Fresnel equation to the following:

\begin{equation}
R = \left(\frac{n_1-n_2}{n_1+n_2}\right)^2
\label{eq:}
\end{equation}
For refractive indexes n1 and n2 are respectively 1 and 1.43 (air, fused silica) which leads to R=3~\% which is quite inaccurate compared to the result from the OTDR -13~\%. Thus there have to be errors in either the measurement or the calculation. The calculation is only valid for rays orthogonal to a material boundary. The modes in a fiber don't propagate as rays but rather as waves. Thus there is no defined angle in which the mode hits the boundary. The reflectivity has to be calculated by a wave model of the fiber.
...
Figure \ref{fig:5_line} shows the measured data. After 10~km of fiber a peak with a sudden drop of power right afterwards can be seen. This leads to the assumption that there is a splitter with at least one open port. The large peak at about 12 km leads to the idea that there is a open PC connector. The small drop at the 17$^{\mathrm{th}}$~km can have several different explanations
\begin{itemize}
	\item A bad connector
	\item An error in the fiber (for example a large bend)
	\item An open APC connector 
\end{itemize}
It can't be told which of the above the event in the backscattering signal corresponds without knowing the network topology. For the detection of faults in the network, the signal is recorded with a working setup. In the case of fault the measured signal is compared to this reference signal. By the differences in the signal the location of the fault can be determined.
...
\caption{Scheme of a possible network topology.}%
\label{fig:5_network}%
\end{figure}


The network topology could be the following:\\
A 10~km spool of fiber followed by a splitter. The drop at the splitter is about 9~dB. A splitting ratio of 1:4 corresponds to 6~dB. Thus it could be a 1:4 splitter with losses of 2~dB. At one port of the splitter is a approximately 800~m long fiber with an open PC at the end. Another port could have a $\sim$9~km long line with an error or a bad connection at the middle of that line. But in that case the reflection at the splitter would be bigger, because there are two open ports. The other case is, that there is one $\sim$9~km long line with no measurable error within the line and an additional line at a third port of the splitter. This third line would than have a length of $\sim$5~km and is ended with an APC connector. This network topology is shown in figure \ref{fig:5_network}.
