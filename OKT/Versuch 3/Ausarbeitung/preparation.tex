...
\caption{Plot of the spatial coherence function}%
\label{fig:fwhm}%
...
To resolve two points the peaks in the signal must be at least $\delta z$ apart. Because the light in the specimen travels the way to the reflecting plane and back in the medium with the refractive index $n$ the distance the light travels is $z=2\cdot n\hat{z}$. From this follows that $\delta z = 2\cdot n\delta \hat{z}$ and
\begin{equation}
\delta \hat{z} = \frac{\delta z}{2n}=\frac{16~\upmu\mathrm{m}}{3} = 5.33~\upmu\mathrm{m}.
\label{eq:resolution}
\end{equation}
...
\begin{equation}
  \delta \hat{z} = \frac{2\pi}{2n\cdot M\cdot \delta k}.
\end{equation}
This calculation is also done with Matlab. The absolute of $\breve{u}(z)_{sr}$ is shown in fig. \ref{fig:profile}. Since $\breve{u}(z)_{sr}$ is symmetric around $z=4$~mm only the range 0$~\leq z\leq~$4~mm is plotted.
...
At $z\approx1.6$~mm the signal shows a good reflecting plane. Figure \ref{fig:profile_zoom} shows the range from 1.4 to 2.5~mm. Here the layer thickness of the tape can be estimated. 
In the signal a periodicity of $\sim$40~$\upmu$ can be found. Because of sampling errors the periodicity is discontinuous. 

The width of one major peak can be estimated as $\sim 40~\upmu$m. So the layer thickness is around 40~$\upmu$m as well.
...
\caption{Interface pattern of a spool of adhesive tape}
\label{fig:profile}
\end{figure}
...
\caption{Zoom of the Interface pattern}
