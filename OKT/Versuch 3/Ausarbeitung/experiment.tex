...
>>>>>>> 5b3da80a481814a42c97c7979e3b40e1e040ff34
...
For each A-scan the z-position of the peak is detected. This is done by finding the maximum of the reflectivity in z-domain. The obtained values are written into a matrix at the corresponding (x,y)-position.

At the last the data is displayed in four surface plots. In the first plot, all A-scans in k-space are shown (cf. fig. \ref{fig:Ak1}). In the second and third plot all A-scans in z-space are shown (cf. fig. \ref{fig:Az1}, \ref{fig:Az_1_trunc}). In the third plot, only the range defined by \textsc{zmin} and \textsc{zmax} is displayed. Lastly in the fourth plot the Matrix calculated earlier is displayed. It shows the 3 dimensional surface of the sample (cf. fig. \ref{fig:3d_1}).


\begin{figure}%
\centering
%\begin{adjustwidth}{0cm}{0cm}
	\subfloat[k-space]{\includegraphics[totalheight=6 cm]{Grafiken/Ak_1.pdf}\label{fig:Ak1}}
	\subfloat[z-space]{\includegraphics[totalheight=4 cm]{Grafiken/Az_1.pdf} \label{fig:Az1}}\\%
	\subfloat[z-space (truncaded)]{\includegraphics[totalheight=4 cm]{Grafiken/Az_trunc1.pdf}\label{fig:Az_1_trunc}}
	\subfloat[3-D surface]{\includegraphics[totalheight=4 cm]{Grafiken/3d_1.pdf} \label{fig:3d_1}}
%\end{adjustwidth}
\caption{Matlab Output. Step size: 1°}%
\label{fig:matlab}%
\end{figure}



The scan was first performed with an angle between -10° to 1° in x-direction and an angle of -7° to 7° in y-direction. The step size was chosen as 1° for a first try. The resolution of the obtained 3D-surface was very low, so a lower step size had to be chosen. When trying a step size of 0.1° the LabView interface crashed. This becomes clear when the amount of data is considered. Each A-scan contains of $n_{A-scan}=$1536 measured values in k-space. There are 111 steps in x-direction and 141 steps in y-direction. Thus for this measurement $111 \cdot 141 \cdot 1536 \approx 24 \cdot  
10^6$ values have to be measured. Obviously the LabView vi cannot handle that much data.
...
%\end{adjustwidth}
\caption{\textbf{a)} Measured profile of a coin with a step size of 0.2°.  \textbf{b)} Austrian 1 ct. coin.}%
\label{fig:Coin}%
\end{figure}

Figure \ref{fig:Profil_3D} shows the result of the measurement with a step size of 0.2°. The depth of the surfaces is color-coded.  For comparison figure \ref{fig:1ct} shows a Austrian 1 cent coin\footnote[1]{Source: European Central Bank} with the section marked that is shown in \ref{fig:Profil_3D}.
