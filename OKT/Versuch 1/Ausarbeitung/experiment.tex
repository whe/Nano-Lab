...
Table \ref{tab:2_ext} shows the external quantum efficiencies of the laser diode for different temperatures and currents, calculated width $\lambda=736~\mathrm{nm}$ (cf. \eqref{eq:lambda}). The external efficiancy decreases with an increasing temperature.

\begin{table}%
\centering
\caption{Quantum efficiencies at different temperatures and currents}
 
\begin{tabular}{cccc}

\toprule
$T$~/~°C& $\eta\i{ext}$(50~mA)&	$\eta\i{ext}$(60~mA)&	$\eta\i{ext}$(65~mA)\\
\midrule
...
For higher temperatures this efficiency did not significantly.
...
This value is not useful since the value is bigger than one. The used values for $\tau\i{P}$ and $\tau\i{R}$ of the preparation could be not valid for the laser diode.
Since $\eta\i{ind}$ depends on the temperature independent $\eta\i{d}$ it does not depend on the temperature as well.
...
Now the photo diode is modulated with a frequency. This is done with a network analyzer (NWA). The modulation frequency is sweapt from 300~kHz to 3~GHz and the Output power is measured. By comparing the Power on the output to the input power, the frequency response of the transmission is determined. The setup of this experiment is shown in figure \ref{fig:measurement3}. Since the measurement PC was not working the frequency response had to be sketched by hand (cf. appendix \ref{app:s21}). 

The measurement of the frequency response was performed at different current levels the frequency $f\i{Kl}$ and the absolute of the frequency overshoot $\left|H\i{Kl}(f\i{Kl})/H\i{Kl}(0)\right|$ was recorded (cf. Tabular \ref{tab:measurement} in the appendix). With these values and the equations \eqref{eq:kleinsignal} and \eqref{eq:maximum} the parameters $\omega\i{r}^2$ and $\gamma\i{r}$ in dependency of the diode current $I_0$ can be calculated. 

The absolute of the frequency overshoot is given by the absolute of \eqref{eq:kleinsignal} at $\omega\i{Kl}$:
\begin{equation}
 \left|H\i{Kl}(\omega\i{Kl})\right| = \frac{\omega\i{r}^2}{\sqrt{(\omega\i{r}^2-\omega\i{Kl}^2)^2 + 4\omega\i{Kl}^2\gamma\i{r}^2}}.
\label{eq:abs_H}
\end{equation}
Next \eqref{eq:maximum} is rearranged to
\begin{equation}
 \gamma\i{r} = \sqrt{\frac{\omega\i{r}^2 - \omega\i{Kl}^2}{2}}
\label{eq:gamma_r}
\end{equation}
and inserted in \eqref{eq:abs_H}: 
\begin{equation}
 \left|H\i{Kl}(\omega\i{Kl})\right| = \frac{\omega\i{r}^2}{\sqrt{(\omega\i{r}^2-\omega\i{Kl}^2)^2 + 2\omega\i{Kl}^2(\omega\i{r}^2 - \omega\i{Kl}^2)}}.
\end{equation}
This can be simplified to:
\begin{equation}
 \left|H\i{Kl}(\omega\i{Kl})\right| = \frac{\omega\i{r}^2}{\sqrt{\omega\i{r}^4- \omega\i{Kl}^4}}.
\end{equation}
Resolving this to $\omega\i{r}^2$ leads to:
\begin{equation}
 \omega\i{r}^2 = \frac{\left|H\i{Kl}(\omega\i{Kl})\right|\cdot\omega\i{Kl}^2}{\sqrt{\left|H\i{Kl}(\omega\i{Kl})\right|^2-1}}
\end{equation}
Inserting in \eqref{eq:gamma_r} leads to:
\begin{equation}
 \gamma\i{r}=\sqrt{\frac{\left|H\i{Kl}(\omega\i{Kl})\right|\cdot\omega\i{Kl}^2}{2\sqrt{\left|H\i{Kl}(\omega\i{Kl})\right|^2-1}}-\frac{\omega\i{Kl}^2}{2}}
\end{equation}

Now $\omega\i{r}^2$ and $\gamma\i{r}$ are calculated for the measured values for $\left|H\i{Kl}(\omega\i{Kl})\right|$ and $\omega\i{Kl}$. The figures \ref{fig:omega2} and \ref{fig:gamma} show the results in dependency of the normalized current $I_0/I\i{th}$.

% For calculating the effective carrier livetime $\tau\i{eff}$ \eqref{eq:omega2_I} can be derrived:
From \eqref{eq:omega2_I} one can see, that the slope of the line in figure \ref{fig:omega2} corresponds to:
\begin{equation}
 \frac{\partial \omega\i{r}^2}{\partial I_0/I\i{th}}=\frac{1}{\tau\i{eff}\tau\i{p}}
\end{equation}
Thus, with the value of $\tau\i{p} = 1.77$~ps from the preparation, $\tau\i{eff}$ can be calculated:
\begin{equation}
 \tau\i{eff}= 1.07\mathrm{~ps}
\end{equation}
...
\begin{figure}%
\centering
 \includegraphics[width=.6\columnwidth]{Grafiken/omega2.pdf}
 \caption{}%
\label{fig:omega2}%
\end{figure}
...
\begin{figure}%
\centering
 \includegraphics[width=.6\columnwidth]{Grafiken/gamma.pdf}
 \caption{}%
\label{fig:gamma}%
\end{figure}
...
%\subfloat[]{\includegraphics[totalheight=6.5	 cm]{Grafiken/gamma.pdf} \label{fig:gamma}}\\%
