...
\begin{equation} \frac{n\cdot 2L}{\lambda_0} = q \hspace{1,5cm}	 q\in \mathbb{N}	
\end{equation}
...
\end{equation}
The length of the resonator $L$ can be computed by
\begin{equation}
 L = \frac{c\cdot\tau\i{U}}{2n} = 310 ~\upmu\mathrm{m}
\end{equation}
Expressed in multiples of the resonator wavelength the length of one resonator circulation is:
\begin{equation}
 q = \frac{n\cdot 2L}{\lambda_0} = 2782
\end{equation}

\subsection{Question 4}
For perpendicular incident light the power reflection coefficient for a boundary between GaAs and air is given by
\begin{equation}
 R = \left(\frac{n\i{air} - n}{n\i{air} + n}\right)^2 = 0.309
\label{eq:reflection}
\end{equation}
The reflection at the mirrors can also be expressed as an equivalent attenuation or time constant, which is calculated by
\begin{equation}
 \alpha\i{R}=-\frac{\ln R}{L}
\end{equation}
and
\begin{equation}
 \tau\i{R}=\frac{n\i{g}}{c\alpha\i{R}} 
 \label{eq:tau_r}
\end{equation}
respectively.

With the $R$ obtained in \eqref{eq:reflection} this leads to $\alpha\i{R}=37.88\mathrm{~cm}^{-1}$ and $\tau\i{R}=3.64\mathrm{~ps}$. With a attenuation of the medium of about $\alpha\i{V}=40$~cm$^{-1}$ the lifetime of a photon is:
\begin{equation}
 \tau\i{p}=\frac{n\i{g}}{c(\alpha\i{R}+\alpha\i{V})}= 1.77 \mathrm{~ps}
 \label{eq:tau_p}
\end{equation}
This is smaller than the circulation time of the resonator with about a factor 4.
...
% 
% \todo{nochmal überlegen was da so passiert}
...
\begin{equation}
\begin{split}
\frac{\partial G}{\partial n\i{T}} =\frac{G(n\i{t})}{r\i{eff}(n\i{T})\tau\i{eff}}\quad,\\
\frac{N\i{P0}}{\tau\i{P}}= \frac{I-I\i{s}}{e} \quad\mathrm{and}\\
\Gamma G(n\i{t})=\frac{1}{\tau\i{P}}
\end{split}
\label{eq:}
\end{equation}
leads to
...
\paragraph{6. b)}
For operation of the laserdiode with low oszillation, the operation point of the diode should be in the region of critical damping. In this case $\omega\i{r}=\gamma\i{r}\sqrt{2}$ and $\omega\i{3 dB}=\omega\i{r}$.
There the resonance overshoot disappears. % for $2\gamma\i{r}>\omega\i{r}$.
Thus the operation point should be chosen in that domain.
