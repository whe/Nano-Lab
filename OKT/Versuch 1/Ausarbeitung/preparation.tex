...
and
\begin{equation}
 \tau\i{R}=\frac{n\i{g}}{c\alpha\i{S}} 
 \label{eq:tau_r}
\end{equation}
respectively.
\todo{n$_g$ muss in zähler bei $\tau_r$}
With the $R$ obtained in \eqref{eq:reflection} this leads to $\alpha\i{S}=37.88\mathrm{~cm}^{-1}$ and $\tau\i{R}=3.64\mathrm{~ps}$. With a attenuation of the medium of about $\alpha\i{V}=40$~cm$^{-1}$ the lifetime of a photon is:
\begin{equation}
 \tau\i{p}=\frac{n\i{g}}{c(\alpha\i{S}+\alpha\i{V})}= 1.77 \mathrm{~ps}
 \label{eq:tau_p}
\end{equation}
